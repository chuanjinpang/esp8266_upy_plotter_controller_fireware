...
import rm_gcode_comment as rmgc
...
class Plotter(object):
    def __init__(self, xyMin=(7,8), xyMax=(204,178),
            drawSpeed=35, moveSpeed=40, zSpeed=5, workZ = 15, liftDeltaZ = 2.5, safeDeltaZ = 20,
...
def parseSVG(svgTree, tolerance=0.05, shader=None, strokeAll=False, pens=None, extractColor = None):
    ...
    for path in parser.getPathsFromSVG(svgTree)[0]:
        ...
        for line in lines:
            #print(line)
            data[strokePen].append([(line[0].real,line[0].imag),(line[1].real,line[1].imag)])
...
""")
    ...
    plotter = Plotter( workZ = 0, liftDeltaZ =3,safeDeltaZ = 4) # Park=workZ+safeDeltaZ lift=workZ+liftDeltaZ
    hpglOut = False
    strokeAll = False
    extractColor = None
    gcodePause = "@pause"
    ...
    if g:
        ...
        #output gcode name 
        gcode_fn=args[0]+".gcode"
        with open(gcode_fn, mode='w') as f:
            f.write('\n'.join(fixComments(plotter, g, comment=plotter.comment)))
            f.close()
        #strip comment
        rmgc.rm_comments(gcode_fn)
    else:
        sys.stderr.write("No points.　just test")
        sys.exit(1)
